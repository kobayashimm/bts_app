#学習済みモデルを組み込んで、実際に動作させるための View ファイル

# 必要なモジュールのインポート
import torch
from bts import transform, Net # bts.py から前処理とネットワークの定義を読み込み
...
def predicts():
    ...
    if request.method == 'POST':
        ...
        if file and allwed_file(file.filename):
            ...
            # 入力された画像に対して推論
            pred = predict(image)
            btsName_ = getName(pred)
            return render_template('result.html', btsName=btsName_, image=base64_data)
        return redirect(request.url)
